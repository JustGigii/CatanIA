...
public class ManngerOnline : MonoBehaviour
{
        ...
        if (!Wating)
        {
            ...
            if (!NowBot)
            {
                if (playing.Count <= 1)
                {
                    ...
                    Place = 1;
                    GameObject Add = ClickChecker.CheakPoint(Board, Command);
                    if (Add != null)
                    {
                ...
                else
                {
                    ...
                    if (NumberOnCube == 7)
                    {
                        ...
                        {
                            ClickChecker.SevenEventRed(red, Command);
                            ClickChecker.SevenEventBlue(blue, Command);
                        }
                        if (red.NumberOfResoures() <= 7 && blue.NumberOfResoures() <= 7)
                        {
                            NumberOnCube = -1;
    ...
    {
        if (Command == this.gameObject.name || NowBot)
        {
            if (playing.Score >= 10)
            {
                Next.active = false;
